"""EDA headline-insights report generation (Phase 2)."""

from __future__ import annotations

import pandas as pd

from src.config import AGE_BUCKET_BINS, AGE_BUCKET_LABELS


def compute_eda_insights(df: pd.DataFrame) -> str:
    """Compute the Phase 2 headline findings and format them as markdown.

    Covers depreciation, mileage, regional spread, brand tiers, value
    heaping, VIN/condition confounds, and the age-year collinearity /
    single-month-snapshot caveats. Each numbered finding maps to one of the
    ...
    Returns:
        str: Full markdown report (docs/phase2_insights.md content).
    """
    ...
    d["age_bucket"] = pd.cut(d["age"], AGE_BUCKET_BINS, labels=AGE_BUCKET_LABELS)
