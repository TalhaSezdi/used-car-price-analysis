...
- Fit on 197,814 rows in **5.5s**. IF is ~O(n log n) via random subsampling; LOF is ~O(n^2) on pairwise distances (~39e9 ops) and One-Class SVM does not scale to ~200k. IF is the only practical full-data choice.
